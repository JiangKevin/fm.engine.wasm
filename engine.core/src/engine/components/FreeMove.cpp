...
void FreeMove::registerWithEngine( Engine* engine )
{
    ...
    input->bindAxis( "seesaws",
                     [ this ]( float value )
                     {
                         m_seesawVelocity = value;
                     } );
}
...
void FreeMove::update( Input* input, std::chrono::microseconds delta )
{
    ...
    if ( m_seesawVelocity != 0 )
    {
        Move( glm::rotate( m_parentEntity->getTransform().getRotation(), glm::vec3( 0.0f, m_seesawVelocity, 0.0f ) ), moveAmount );
    }

...
void CoolGame::init(GLManager *glManager) {
  ...
  input->bindAction("fire", IE_PRESSED, [this]() {
    ...
    cube.getEntity()->getTransform().setPosition(
        primary_camera->getParent()->getPosition());
    ...
    cube.getEntity()->getComponent<BoxCollider>()->applyCentralImpulse(
        glm::vec3(dir.x * 500.0f, dir.y * 500.0f, dir.z * 500.0f));
  });
  ...
  auto brickMat = std::make_shared<Material>(
      std::make_shared<Texture>(Asset("bricks2.jpg")),
  ...
  money.getEntity()->addComponent<PerspectiveCamera>(
      glm::pi<float>() / 2.0f,
      getEngine()->getWindow()->getWidth() /
  ...
  money2.getEntity()->addComponent<PerspectiveCamera>(
      glm::pi<float>() / 2.0f,
      getEngine()->getWindow()->getWidth() /
          (float)getEngine()->getWindow()->getHeight(),
      0.8f, 100.0f);
  ...
  money2.getEntity()
      ...
      .setScale(glm::vec3(0.8, 0.8, 0.8));
  money2.getEntity()->addComponent<SpotLight>(
